import Router, { application, response, type Request, type Response } from 'express';
...
userRouter.post('/signup',async(req:Request,res:Response)=>{
    try {
        ...
        if(user)
        {
            res.status(403).json({message:'User already exsist with this username'})
        }
...
userRouter.post('/refresh',(req:Request,res:Response)=>{
    ...
    try {
        const decodedData = jwt.verify(refreshToken,Refresh_Secret) as {id:string}
        const accessToken = jwt.sign({id:decodedData.id},Access_Secret,{expiresIn:'15m'})
...
userRouter.get('/profile',userAuth,async(req:AuthRequest,res:Response)=>{
